...
class GenerateJSON extends Config{
  ...
  function __construct(){
    ...
    if(!empty($dirName)){
      ...
      $viewDirName = $dirName;
      $triggerDirName = $dirName;
      $routineDirName = $dirName;
    }
    $arrayTable = $this->showStruktur($tableName,$tableDirName);
    $arrayView = $this->showView($viewName,$viewDirName);
    $arrayTrigger = $this->showTrigger($triggerName,$triggerDirName);
    $arrayRoutine = $this->showRoutine($routineName,$routineDirName);
    $arrayJSON[] = array(
      "databaseName" => $databaseName,
      "tables" => $arrayTable,
      "views" => $arrayView,
      "triggers" => $arrayTrigger,
      "routines" => $arrayRoutine,
    );
    // echo json_encode($arrayJSON,JSON_PRETTY_PRINT);
    echo str_replace("\/","/",json_encode($arrayJSON,JSON_PRETTY_PRINT));
  ...
  function showStruktur($tableName,$tableDirName){
    ...
    for ($i=0; $i < sizeof($explodeTableName); $i++) {
      if($this->filterExtension($explodeTableName[$i]) == 0 && $this->sqlRowCount($this->sqlQuery("SHOW CREATE VIEW ".$explodeTableName[$i])) == 0){
        $arrayTableName[] = array(
          "table_name" => $explodeTableName[$i],
          "json_file" => $tableDirName."/".$explodeTableName[$i].".json",
        );
      }
    }
  ...
  function showTrigger($triggerName,$dirName){
    ...
    for ($i=0; $i < sizeof($explodeTriggerName); $i++) {
      if($this->filterExtension($explodeTriggerName[$i]) == 0){
        $arrayTrigger[] = array(
          "trigger_name" => $explodeTriggerName[$i],
          "json_file" => $dirName."/".$explodeTriggerName[$i].".json",
        );
      }
    }
  ...
  function showRoutine($routineName,$dirName){
    ...
    for ($i=0; $i < sizeof($explodeRoutineName); $i++) {
      if($this->filterExtension($explodeRoutineName[$i]) == 0){
        $arrayRoutine[] = array(
          "routine_name" => $explodeRoutineName[$i],
          "json_file" => $dirName."/".$explodeRoutineName[$i].".json",
        );
      }
    }
    return $arrayRoutine;
  ...
  function showView($viewName,$dirName){
    ...
    for ($i=0; $i < sizeof($explodeViewName); $i++) {
      if($this->filterExtension($explodeViewName[$i]) == 0){
        $arrayView[] = array(
          "view_name" => $explodeViewName[$i],
          "json_file" => $dirName."/".$explodeViewName[$i].".json",
